...
class KNNTest extends PHPUnit_TestCase {
    ...
    public function testInitCentroids() {
        ...
        $shown_up = array();
        for ($i = 0; $i < 300; $i++) {
            $centroids = Kmeans::_ll_init_centroids($xs, 2);
            ...
            $this->assertFalse(Kmeans::_ll_init_centroids($xs, 100));
        }
    ...
    public function testFindClosestCentroid() {
        ...
        $this->assertEquals(0, (Kmeans::_ll_closest_centroid($x, $centroids)));
        for ($i = 0; $i < 100; $i++) {
            shuffle($centroids);
            $expected = array_search(array(2, 2, 2, 2), $centroids);
            $this->assertEquals($expected, (Kmeans::_ll_closest_centroid($x, $centroids)));
        }
...
$suite->addTest(new KNNTest('testInitCentroids'));

$suite->addTest(new KNNTest('testDistanceToCentroid'));

$suite->addTest(new KNNTest('testFindClosestCentroid'));

$suite->addTest(new KNNTest('testFlip'));
